...
var controllername_tree = "${pageContext.request.contextPath }/projectController.do";
var trlen,xdnf; 
var nd = '<%=nd%>';
var parent =  '<%=id%>';
...
$(document).ready(function(){
	...
	//调用ajax插入
	defaultJson.doQueryJsonList(controllername+"?queryXdxmk",data,DT1,"query_ready_zc",true);
});
...
function query_ready_zc()
{
	...
	//调用ajax插入
	defaultJson.doQueryJsonList(controllername_tree+"?getChildProject&parent="+parent,data,DT2,"query_ready_js",true);
}
...
//保存导入子项目
function insert_zxm(ids)
{
	doInsertJson_zxm(controllername_tree + "?addZxm&ids="+ids+"&parent="+parent+"&nd="+nd, null,null,"insert");
}
//插入
doInsertJson_zxm = function(actionName, data1,tablistID,callbackFunction) {
var success  = true;
	...
	$.ajax({
		...
		success : function(result) {
			var success_flag=result.msg;
			xAlert("信息提示",success_flag);
			
			},
	    error : function(result) {
	    	xAlert("信息提示",result.msg,'2');
...
$(function() {
	...
	$("#btnQuery").click(function() {
		var data=combineQuery.getQueryCombineData(queryForm,frmPost,DT1);
		//调用ajax插入
		defaultJson.doQueryJsonList(controllername+"?queryXdxmk",data,DT1,null,true);
	});
...
function getXmmcQueryCondition(){
	var initData = '{"querycondition":{"conditions":[ {"value": "0","fieldname":"0","operation":"=","logic":"and"},]},"orders":[{"order":"desc","fieldname":"pxh"}]}';
	...
	//年度
		...
		var defineCondition = {"value": "<%=nd%>","fieldname":"ND","operation":"=","logic":"and"};
		jsonData.querycondition.conditions.push(defineCondition);
...
<div class="container-fluid">
	...
	<div class="row-fluid">
   			...
   			<table class="B-table" width="100%">
		      	...
		      	<TR style="display:none;">
					...
					<TD class="right-border bottom-border">
						...
						<INPUT type="text" class="span12" kind="text" id="nd" fieldname="t.nd" value="<%=nd%>" operation="="/>
     			...
     			<!--可以再此处加入hidden域作为过滤条件 -->
       			<tr>
					...
					<td width="25%" colspan="3"  class="right-border bottom-border">
						<input class="span12" type="text" placeholder="" name="XMMC" check-type="maxlength" maxlength="500" fieldname="t.XMMC" operation="like" id="XMMC" autocomplete="off">
					</td>					
		...
		<div class="B-small-from-table-autoConcise">
			<h4>
				待选择项目列表&nbsp;&nbsp; 共<font style=" margin-left:5px; margin-right:5px;;font-size:28px;color:red;">0</font>个
