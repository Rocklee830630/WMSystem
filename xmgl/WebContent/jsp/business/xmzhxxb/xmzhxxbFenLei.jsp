  ...
  //		$("#DT1").attr("pageNum", pageNum);
  	...
  	$(document).ready(function() {
		...
		setDefaultNd();
  		...
  		defaultJson.doQueryJsonList(controllername+"?queryXmzhxxFenLei", data, DT1, "setTdBackColor", true);
  	...
  	//		setPageHeight();
	      ...
	      //  	alert(dataObj);
  			...
  			//调用ajax插入
  	 		defaultJson.doQueryJsonList(controllername+"?queryXmzhxxFenLei",data, DT1, "setTdBackColor", false);
  	 	});  
  	...
  	function  setTotalPage(querycondition,queryResult)
	{
  	...
  	//设置表格合计
  	function queryAfterSum(){

		
		var tzgcsum = 0;//年度总投资估算 工程
		var tzzcsum = 0;//年度总投资估算 征拆
		var tzqtsum = 0;//年度总投资估算 其他
		var tzztzsum = 0;//年度总投资估算 总投资
		
		var zttzgcsum = 0;//总体投资估算 工程
		var zttzzcsum = 0;//总体投资估算 征拆
		var zttzqtsum = 0;//总体投资估算 其他
		var zttzztzsum = 0;//总体投资估算 总投资
		
		var GS_GC = 0;// 概算信息 工程
		var GS_ZC = 0;// 概算信息 征拆
		var GS_QT = 0;// 概算信息 其他
		var GS_ZTZ = 0;// 概算信息 总投资
		
		var ZJZF_GCFY = 0;// 资金支付信息 工程
		var ZJZF_ZCFY = 0;// 资金支付信息 征拆
		var ZJZF_QTFY = 0;// 资金支付信息 其他
		var ZJZF_ZZFFY = 0;// 资金支付信息 总投资
		
		var JS_GCJE = 0;// 结算信息 工程
		var JS_ZCJE = 0;// 结算信息 征拆
		var JS_QTJE = 0;// 结算信息 其他
		var JS_ZJSJE = 0;// 结算信息总投资

		
		//计算合计值(当前页面)
		//获取行数
		 var pagerowslength = $("#DT1").getTableRows();
		 var tr_obj = $("tbody tr" ,$("#DT1"))[0];
		 //tr_obj.removeAttribute("rowJson")
		 //alert(tr_obj.outerHTML)
		  for(var i =0 ; i < pagerowslength;i++){
			var rowobj = $("#DT1").getRowJsonObjByIndex(i);
			if(rowobj.LY=="0"){
				if(rowobj.TZ_GC != "")
				{
					tzgcsum += parseFloat(rowobj.TZ_GC);
				}
				if(rowobj.TZ_ZC != "")
				{
					tzzcsum += parseFloat(rowobj.TZ_ZC);
				}
				if(rowobj.TZ_QT != "")
				{
					tzqtsum += parseFloat(rowobj.TZ_QT);
				}
				if(rowobj.TZ_ZTZ != "")
				{
					tzztzsum += parseFloat(rowobj.TZ_ZTZ);
				}
				
				if(rowobj.ZTTZ_GC != "")
				{
					zttzgcsum += parseFloat(rowobj.ZTTZ_GC);
				}
				if(rowobj.ZTTZ_ZC != "")
				{
					zttzzcsum += parseFloat(rowobj.ZTTZ_ZC);
				}
				if(rowobj.ZTTZ_QT != "")
				{
					zttzqtsum += parseFloat(rowobj.ZTTZ_QT);
				}
				if(rowobj.ZTTZ_ZTZ != "")
				{
					zttzztzsum += parseFloat(rowobj.ZTTZ_ZTZ);
				}
				
				if(rowobj.GS_GC != "")
				{
					GS_GC += parseFloat(rowobj.GS_GC);
				}
				if(rowobj.GS_ZC != "")
				{
					GS_ZC += parseFloat(rowobj.GS_ZC);
				}
				if(rowobj.GS_QT != "")
				{
					GS_QT += parseFloat(rowobj.GS_QT);
				}
				if(rowobj.GS_ZTZ != "")
				{
					GS_ZTZ += parseFloat(rowobj.GS_ZTZ);
				}
				
				if(rowobj.ZJZF_GCFY != "")
				{
					ZJZF_GCFY += parseFloat(rowobj.ZJZF_GCFY);
				}
				if(rowobj.ZJZF_ZCFY != "")
				{
					ZJZF_ZCFY += parseFloat(rowobj.ZJZF_ZCFY);
				}
				if(rowobj.ZJZF_QTFY != "")
				{
					ZJZF_QTFY += parseFloat(rowobj.ZJZF_QTFY);
				}
				if(rowobj.ZJZF_ZZFFY != "")
				{
					ZJZF_ZZFFY += parseFloat(rowobj.ZJZF_ZZFFY);
				}
				
				if(rowobj.JS_GCJE != "")
				{
					JS_GCJE += parseFloat(rowobj.JS_GCJE);
				}
				if(rowobj.JS_ZCJE != "")
				{
					JS_ZCJE += parseFloat(rowobj.JS_ZCJE);
				}
				if(rowobj.JS_QTJE != "")
				{
					JS_QTJE += parseFloat(rowobj.JS_QTJE);
				}
				if(rowobj.JS_ZJSJE != "")
				{
					JS_ZJSJE += parseFloat(rowobj.JS_ZJSJE);
				}
		...
		//格式化结果
		tzgcsum = commafy(tzgcsum);
		tzzcsum = commafy(tzzcsum);
		tzqtsum = commafy(tzqtsum);
		tzztzsum = commafy(tzztzsum); 
		
		zttzgcsum = commafy(zttzgcsum);
		zttzzcsum = commafy(zttzzcsum);
		zttzqtsum = commafy(zttzqtsum);
		zttzztzsum = commafy(zttzztzsum); 
		
		GS_GC = commafy(GS_GC);
		GS_ZC = commafy(GS_ZC);
		GS_QT = commafy(GS_QT);
		GS_ZTZ = commafy(GS_ZTZ); 
		
		ZJZF_GCFY = commafy(ZJZF_GCFY);
		ZJZF_ZCFY = commafy(ZJZF_ZCFY);
		ZJZF_QTFY = commafy(ZJZF_QTFY);
		ZJZF_ZZFFY = commafy(ZJZF_ZZFFY); 
		
		JS_GCJE = commafy(JS_GCJE);
		JS_ZCJE = commafy(JS_ZCJE);
		JS_QTJE = commafy(JS_QTJE);
		JS_ZJSJE = commafy(JS_ZJSJE); 
		
		var sumrow = $("tfoot tr" ,$("#DT1"));
		
		var temptr = "<tr style=\"margin:0 auto; padding:0px; border-spacing:0px; border-style:none; border-collapse:collapse;\">"+
		  "<td><b>合计</b>"+
		  "</td>"+
		  "<td></td>"+
		  "<td></td>"+
		  ...
		  "<td align = \"right\"><b>"+tzgcsum+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+tzzcsum+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+tzqtsum+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+tzztzsum+
		  "</b></td>"+
		  
		  "<td align = \"right\"><b>"+zttzgcsum+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+zttzzcsum+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+zttzqtsum+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+zttzztzsum+
		  "</b></td>"+
		  
		  "<td align = \"right\"><b>"+GS_GC+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+GS_ZC+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+GS_QT+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+GS_ZTZ+
		  "</b></td>"+
		  
		  "<td align = \"right\"><b>"+ZJZF_GCFY+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+ZJZF_ZCFY+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+ZJZF_QTFY+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+ZJZF_ZZFFY+
		  "</b></td>"+
		  
		  "<td align = \"right\"><b>"+JS_GCJE+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+JS_ZCJE+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+JS_QTJE+
		  "</b></td>"+
		  "<td align = \"right\"><b>"+JS_ZJSJE+
		  "</b></td>"+
		
		  "</tr>";
			  
			sumrow.replaceWith(temptr); 
...
function doSjrws(obj){
	...
	if(ly=="1")
		return "";
	var sjrws=obj.SJ_SJRWS;
	var value=sjrws?('<i class="icon-yes-green" title="有设计任务书"></i>'):('<i class="icon-no-red" title="无设计任务书"></i>');
	return value;
}
/******************************** 
 * 设计-end
 ...
 ********************************/
 ...
 /***************** 概算信息 END*********************/
  ...
  /*****************  资金支付信息 START*********************/
function doZJZF_GCFY(obj){
	if(obj.LY==1){
		 return "<b>"+obj.ZJZF_GCFY_SV+"</b>";
	 }else
		 return obj.ZJZF_GCFY_SV;
}
function doZJZF_ZCFY(obj){
	if(obj.LY==1){
		 return "<b>"+obj.ZJZF_ZCFY_SV+"</b>";
	 }else
		 return obj.ZJZF_ZCFY_SV;
}
function doZJZF_QTFY(obj){
	if(obj.LY==1){
		 return "<b>"+obj.ZJZF_QTFY_SV+"</b>";
	 }else
		 return obj.ZJZF_QTFY_SV;
}
function doZJZF_ZZFFY(obj){
	if(obj.LY==1){
		 return "<b>"+obj.ZJZF_ZZFFY_SV+"</b>";
	 }else
		 return obj.ZJZF_ZZFFY_SV;
}
...
function doJS_ZCJE(obj){
	if(obj.LY==1){
		 return "<b>"+obj.JS_ZCJE_SV+"</b>";
	 }else
		 return obj.JS_ZCJE_SV;
}
function doJS_QTJE(obj){
	if(obj.LY==1){
		 return "<b>"+obj.JS_QTJE_SV+"</b>";
	 }else
		 return obj.JS_QTJE_SV;
}
function doJS_ZJSJE(obj){
	if(obj.LY==1){
		 return "<b>"+obj.JS_ZJSJE_SV+"</b>";
	 }else
		 return obj.JS_ZJSJE_SV;
}
...
getWinData = function(data){
	...
	//调用ajax插入
	defaultJson.doQueryJsonList(controllername+"?queryXmzhxxFenLei",moreQueryData, DT1, "setTdBackColor", false);
	isMore = false;
};
			...
			<table width="100%" class="table-hover table-activeTd B-table" id="DT1" type="single" noPage="true" pageNum="1000">
                <thead>		                		
                    <tr>
                     	...
                     	<th colindex=2  fieldname="XMS" rowspan="2" tdalign="center" CustomFunction="doxms">大项</th>	
                     	<th colindex=3  fieldname="ZXMS" rowspan="2" tdalign="center" >子项</th>	
						<th colindex=4  fieldname="XMBH" rowspan="2" tdalign="center" rowMerge="true" hasLink="true" linkFunction="rowView">&nbsp;项目编号&nbsp;</th>
						<th colindex=5  fieldname="XMMC" rowspan="2" maxlength="20" rowMerge="true" CustomFunction="doXmmc">&nbsp;项目名称&nbsp;</th>
						<th colindex=6  fieldname="BDMC" rowspan="2" maxlength="70" CustomFunction="doBdmc">&nbsp;标段名称&nbsp;</th>
						<th colindex=7  fieldname="JKZK" rowspan="2" CustomFunction="doJkzk" noprint="true">&nbsp;健康&nbsp;<br>&nbsp;状况&nbsp;</th>
                     	<th colindex=8  fieldname="GTT" rowspan="2" CustomFunction="doGtt" noprint="true">&nbsp;甘特图&nbsp;</th>
						<th colindex=9  fieldname="JSDZ" rowspan="2" maxlength="9">&nbsp;建设位置&nbsp;</th>
						<th colindex=10  fieldname="XXJT" rowspan="2" tdalign="center" CustomFunction="doDz"></th>
						<th colindex=11  fieldname="XMGLGS" rowspan="2" maxlength="9">&nbsp;项管公司&nbsp;</th>
						<th colindex=12 fieldname="XMXZ" rowspan="2" maxlength="9" tdalign="center">&nbsp;项目性质&nbsp;</th>
						<th colindex=13 fieldname="NDMB" rowspan="2" maxlength="9">&nbsp;年度目标&nbsp;</th>
						<th colspan="5">&nbsp;工程进展&nbsp;</th>
						<th colspan="4">&nbsp;手续办理&nbsp;</th>
						<th colspan="6">&nbsp;设计进展&nbsp;</th>
						<th colindex=29 fieldname="LBJ" rowspan="2" maxlength="15" tdalign="right" CustomFunction="doLBJ">&nbsp;拦标价（元）&nbsp;</th>
                    ...
                    <tr>
                    	<!-- 工程进展 -->
						<th colindex=14 tdalign="center" fieldname="GC_YCQ" CustomFunction="doZC">&nbsp;已拆迁&nbsp;</th>
						<th colindex=15 tdalign="center" fieldname="GC_YPQ" CustomFunction="doPQ">&nbsp;已排迁&nbsp;</th>
						<th colindex=16 tdalign="center" fieldname="GC_YKG" CustomFunction="doKG">&nbsp;已开工&nbsp;</th>
						<th colindex=17 tdalign="center" fieldname="GC_YWG" CustomFunction="doWG">&nbsp;已完工&nbsp;</th>
						<th colindex=18 tdalign="center" fieldname="GC_YJJG" CustomFunction="doJG">&nbsp;已交/竣工&nbsp;</th>
						
                    	<!-- 手续办理 -->
						<th colindex=19 tdalign="center" fieldname="SXBL_KYPF" CustomFunction="doKYPF">&nbsp;可研批复&nbsp;</th>
						<th colindex=20 tdalign="center" fieldname="SXBL_TDHB" CustomFunction="doHBJDS">&nbsp;土地划拨&nbsp;</th>
						<th colindex=21 tdalign="center" fieldname="SXBL_GCGHXK" CustomFunction="doGCGHXKZ">&nbsp;工程规划许可&nbsp;</th>
						<th colindex=22 tdalign="center" fieldname="SXBL_SGXK" CustomFunction="doSGXK">&nbsp;施工许可&nbsp;</th>
							
                    	<!-- 设计进展 -->
						<th colindex=23 tdalign="center" fieldname="SJ_SJRWS" CustomFunction="doSjrws">&nbsp;设计任务书&nbsp;</th>
						<th colindex=24 tdalign="center" fieldname="SJ_CSPF" CustomFunction="doCBSJPF">&nbsp;初设批复&nbsp;</th>
						<th colindex=25 tdalign="center" fieldname="SJ_CQT" CustomFunction="doCQT">&nbsp;拆迁图&nbsp;</th>
						<th colindex=26 tdalign="center" fieldname="SJ_PQT" CustomFunction="doPQT">&nbsp;排迁图&nbsp;</th>
						<th colindex=27 tdalign="center" fieldname="SJ_SGT" CustomFunction="doSGT">&nbsp;施工图&nbsp;</th>
						<th colindex=28 tdalign="right"  fieldname="SJ_BGS">&nbsp;变更数&nbsp;</th>
							
                    	<!-- 招标进展 -->
						<th colindex=30 fieldname="ZB_SJDW" maxlength="9" CustomFunction="doZB_SJDW">&nbsp;设计单位&nbsp;</th>
						<th colindex=31 fieldname="ZB_JLDW" maxlength="9" CustomFunction="doZB_JLDW">&nbsp;监理单位&nbsp;</th>
						<th colindex=32 fieldname="ZB_SGDW" maxlength="9" CustomFunction="doZB_SGDW">&nbsp;施工单位&nbsp;</th>
							
                    	<!-- 排迁信息 -->
						<th colindex=33 tdalign="right" fieldname="PQ_RWS_WCS">&nbsp;排迁任务数/完成数&nbsp;</th>
						<th colindex=34 tdalign="right" fieldname="PQ_ZHTJE">&nbsp;总合同金额&nbsp;</th>
						<th colindex=35 tdalign="right" fieldname="PQ_YZF">&nbsp;已支付&nbsp;</th>
						<th colindex=36 tdalign="right" fieldname="PQ_BFB">&nbsp;%&nbsp;</th>
							
                    	<!-- 征收信息 -->
						<th colindex=37 tdalign="right" fieldname="ZS_ZJMS_JWCQ" CustomFunction="showJm">&nbsp;总居民数/已完拆迁&nbsp;</th>
						<th colindex=38 tdalign="right" fieldname="ZS_ZQYS_YWCQ" CustomFunction="showQy">&nbsp;总企业数/已完拆迁&nbsp;</th>
						<th colindex=39 tdalign="right" fieldname="ZS_ZDMJ_YWZD" CustomFunction="showZd">&nbsp;征地面积（m2）/已完征地&nbsp;</th>
						<th colindex=40 tdalign="right" fieldname="ZS_BRZJ" CustomFunction="showBrzj">&nbsp;拨入资金&nbsp;</th>
						<th colindex=41 tdalign="right" fieldname="ZS_YSYZJ" CustomFunction="showSyzj">&nbsp;已使用资金&nbsp;</th>
						<th colindex=42 tdalign="right" fieldname="ZS_YE" CustomFunction="showYe">&nbsp;余额&nbsp;</th>
							
                    	<!-- 合同信息 -->
						<th colindex=43 tdalign="right" fieldname="HT_JQHTS">&nbsp;已签合同数&nbsp;</th>
						<th colindex=44 tdalign="right" fieldname="HT_ZSHTJE">&nbsp;征收合同（虚拟）金额&nbsp;</th>
						<th colindex=45 tdalign="right" fieldname="HT_PQHTJE">&nbsp;排迁合同金额&nbsp;</th>
						<th colindex=46 tdalign="right" fieldname="HT_SJHTJE">&nbsp;设计合同金额&nbsp;</th>
						<th colindex=47 tdalign="right" fieldname="HT_JLHTJE">&nbsp;监理合同金额&nbsp;</th>
						<th colindex=48 tdalign="right" fieldname="HT_SGHTJE">&nbsp;施工合同金额&nbsp;</th>
						<th colindex=49 tdalign="right" fieldname="HT_QTHTJE">&nbsp;其他合同金额&nbsp;</th>
						<th colindex=50 tdalign="right" fieldname="HT_ZHTJE">&nbsp;合同总金额&nbsp;</th>
						<th colindex=51 tdalign="right" fieldname="HT_WCTZ" CustomFunction="showHT_WCTZ">&nbsp;完成投资&nbsp;</th>
						<th colindex=52 tdalign="right" fieldname="HT_BFB1">&nbsp;%&nbsp;</th>
						<th colindex=53 tdalign="right" fieldname="HT_YZF" CustomFunction="showHT_YZF">&nbsp;已支付&nbsp;</th>
						<th colindex=54 tdalign="right" fieldname="HT_BFB2">&nbsp;%&nbsp;</th>
							
                    	<!-- 投资估算（万元） -->
						<th colindex=55 tdalign="right" fieldname="TZ_GC" CustomFunction="doTZ_GC">&nbsp;工程&nbsp;</th>
						<th colindex=56 tdalign="right" fieldname="TZ_ZC" CustomFunction="doTZ_ZC">&nbsp;征拆&nbsp;</th>
						<th colindex=57 tdalign="right" fieldname="TZ_QT" CustomFunction="doTZ_QT">&nbsp;其他&nbsp;</th>
						<th colindex=58 tdalign="right" fieldname="TZ_ZTZ" CustomFunction="doTZ_ZTZ">&nbsp;总投资&nbsp;</th>
							
                    	<!-- 总体投资估算（万元） -->
						<th colindex=59 tdalign="right" fieldname="ZTTZ_GC" CustomFunction="doZTTZ_GC">&nbsp;工程&nbsp;</th>
						<th colindex=60 tdalign="right" fieldname="ZTTZ_ZC" CustomFunction="doZTTZ_ZC">&nbsp;征拆&nbsp;</th>
						<th colindex=61 tdalign="right" fieldname="ZTTZ_QT" CustomFunction="doZTTZ_QT">&nbsp;其他&nbsp;</th>
						<th colindex=62 tdalign="right" fieldname="ZTTZ_ZTZ" CustomFunction="doZTTZ_ZTZ">&nbsp;总投资&nbsp;</th>
							
                    	<!-- 概算信息（元） -->
						<th colindex=63 tdalign="right" fieldname="GS_GC" CustomFunction="doGS_GC">&nbsp;工程&nbsp;</th>
						<th colindex=64 tdalign="right" fieldname="GS_ZC" CustomFunction="doGS_ZC">&nbsp;征拆&nbsp;</th>
						<th colindex=65 tdalign="right" fieldname="GS_QT" CustomFunction="doGS_QT">&nbsp;其他&nbsp;</th>
						<th colindex=66 tdalign="right" fieldname="GS_ZTZ" CustomFunction="doGS_ZTZ">&nbsp;总投资&nbsp;</th>
							
                    	<!-- 资金支付信息（元） -->
						<th colindex=67 tdalign="right" fieldname="ZJZF_GCFY" CustomFunction="doZJZF_GCFY">&nbsp;工程费用&nbsp;</th>
						<th colindex=68 tdalign="right" fieldname="ZJZF_ZCFY" CustomFunction="doZJZF_ZCFY">&nbsp;征拆费用&nbsp;</th>
						<th colindex=69 tdalign="right" fieldname="ZJZF_QTFY" CustomFunction="doZJZF_QTFY">&nbsp;其他费用&nbsp;</th>
						<th colindex=70 tdalign="right" fieldname="ZJZF_ZZFFY" CustomFunction="doZJZF_ZZFFY">&nbsp;总支付费用&nbsp;</th>
							
                    	<!--  结算信息（元） -->
						<th colindex=71 tdalign="right" fieldname="JS_GCJE" CustomFunction='doGCJE'>&nbsp;工程金额&nbsp;</th>
						<th colindex=72 tdalign="right" fieldname="JS_ZCJE" CustomFunction="doJS_ZCJE">&nbsp;征拆金额&nbsp;</th>
						<th colindex=73 tdalign="right" fieldname="JS_QTJE" CustomFunction="doJS_QTJE">&nbsp;其他金额&nbsp;</th>
						<th colindex=74 tdalign="right" fieldname="JS_ZJSJE" CustomFunction="doJS_ZJSJE">&nbsp;总结算金额&nbsp;</th>
                	</tr>
                </thead> 
              	<tbody></tbody>
               <tfoot>
