...
<body>
	...
	var ganttData ;
		(function(jQuery) {
			...
			function build(options) {
				...
				function build() {
					...
					var minDays = Math
							.floor((opts.slideWidth / opts.cellWidth) + 5);
					var startEnd = DateUtils.getBoundaryDatesFromData(
							opts.data, minDays);
					...
					els.each(function() {
						...
						var w = jQuery("div.ganttview-vtheader", container)
								.outerWidth()
								+ jQuery("div.ganttview-slide-container",
			...
			function handleMethod(method, value) {
				...
				if (method == "setSlideWidth") {
					...
					div.each(function() {
						var vtWidth = $("div.ganttview-vtheader", div)
								.outerWidth();
			...
			var Chart = function(div, opts) {
				...
				var monthNames = [ "1月", "2月", "3月", "4月", "5月", "6月", "7月",
						"8月", "9月", "10月", "11月", "12月" ];
						...
						itemDiv
								.append(jQuery(
										"<div>",
										{
											...
											"css" : {
												"height" : (data[i].series.length * cellHeight)
														+ "px"
											}
										}).append(data[i].name));
					...
					for ( var i = 0; i < data.length; i++) {
						for ( var j = 0; j < data[i].series.length; j++) {
							...
							var size = DateUtils.daysBetween(series.start,
									series.end) + 1;
							//alert(size);
							var offset = DateUtils.daysBetween(start,
									series.start);
							...
							//modified by songxb@2013-08-06 begin
							//日期为空null或""时,图表显示异常处理
		                    if(series.start==null||series.end==null||series.start=='null'||series.end=='null'||series.start.length<=0||series.end.length<=0)
		                    	//开始日期，结束日期为空null或""时,不显示表格
		                    	;
		                    else
			...
			var Behavior = function(div, opts) {
					...
					jQuery("div.ganttview-block", div).resizable(
							{
								...
								stop : function() {
									...
									updateDataAndPosition(div, block,
											cellWidth, startDate);
									if (callback) {
				...
				function updateDataAndPosition(div, block, cellWidth, startDate) {
					...
					var offset = block.offset().left - container.offset().left
							- 1 + scroll;
					...
					block.data("block-data").end = newStart.clone().addDays(
							numberOfDays);
					jQuery("div.ganttview-block-text", block).text(
							numberOfDays + 1);

					// Remove top and left properties to avoid incorrect block positioning,
					// set position to relative to keep blocks relative to scrollbar when scrolling
					block.css("top", "").css("left", "").css("position",
							"relative").css("margin-left", offset + "px");
				}
			...
			var DateUtils = {
				...
				daysBetween : function(start, end) {
					if (!start || !end || start=='null' || end == 'null') { //modfied by songxb@2013-08-06
						return 0;
					}
				...
				getBoundaryDatesFromData : function(data, minDays) {
					...
					for ( var i = 0; i < data.length; i++) {
						for ( var j = 0; j < data[i].series.length; j++) {
							...
							//	maxEnd = end; 
							//}
							if(start!=null&&minStart!=null) //当开始时间为空时不对表格最小日期进行调整
								if (minStart.compareTo(start) == 1) {
									minStart = start;
								}
							if(end!=null&&maxEnd!=null)     //当结束时间为空时不对表盒最大日期进行调整
		 ...
		 /*   ganttData = [ {
			...
			series : [ {
				name : "计划开工时间",
				start :'Tue Aug 20 00:00:00 CST 2013',
				end : 'Tue Aug 20 00:00:00 CST 2013'
			}, {
				name : "实际开工时间",
				start : 'Tue Aug 20 00:00:00 CST 2013',
				end : 'Tue Aug 20 00:00:00 CST 2013',
		...
		function init() {
			var data = null;
			submit(controllername+"?ggtById&id="+id,data,tabList);
