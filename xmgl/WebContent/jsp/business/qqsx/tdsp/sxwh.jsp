...
<script type="text/javascript" charset="utf-8">
  var controllername= "${pageContext.request.contextPath }/qqsx/tdsp/sxfjController.do";
	...
	$(function() {
		...
		$("#feedback").click(function(){
			...
			$('#ConfirmYesButton').one("click",function(){
				...
				//var xmmc=$(json).attr("XMMC");
				$(window).manhuaDialog({"title":"土地审批手续>信息反馈","type":"text","content":"${pageContext.request.contextPath}/jsp/business/qqsx/tdsp/xxfk.jsp","modal":"3"});
			}); 
		});
	});
	//插入或删除使用异步操作
    ...
    //清空表单 主要是ywid的删除 附件的删除 以及字典选择导致fjlb的变换 字典可用的判断
     ...
     function set_Value(){
 		var json=do_value();
 		setValueByArr(json,["JHSJID","XMMC","XMID","SJWYBH"]);
 		...
 		$("input[type='checkbox']").each(function(){
 			...
 			if(check){
 				//alert($(this).attr('sv')+"|"+$(this).attr('value')+"|"+$(this).attr('title'));
 		...
 		$("#Q_FJLX").val(sx);
 		$("#FJLX").attr("src","T#fs_dic_tree:dic_code:dic_value:parent_id='1000000000106' and dic_code in("+sx+")");
 		$("#FJLX").html('');
     ...
     <form method="post" id="SxfjForm"  >
      <table class="B-table" width="100%">
         ...
         <!-- <TR> -->
			...
			<TD><input class="span12" id="BBLSX" type="checkbox" kind="dic" src="TDSPSX" name ="BBLSX" ></TD>
        ...
        <tr>
          ...
          <select class="span12" id="FJLX" name="FJLX" fieldname ="FJLX" kind="dic" placeholder="必填" check-type="required" src="TDSPSX" onchange="change()">
            </select>
          </td>
