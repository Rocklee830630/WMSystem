  ...
  var controllername= "${pageContext.request.contextPath }/qqsx/sgxk/sxfjController.do";
	...
	$(function() {
		...
		$("#save").click(function() {
			...
			if(fjid==null||fjid=="")
				{
					defaultJson.doInsertJson(controllername + "?insertSxfj&ywid="+$("#ywid").val()+"&bdid="+bdid, data1,DT1,"in_callback");
				}else{
					defaultJson.doUpdateJson(controllername + "?updateSxfj&bdid="+bdid, data1,DT1,"up_callback");
				}
			}else{
		...
		$("#feedback").click(function(){
			...
			$('#ConfirmYesButton').one("click",function(){
				...
				var sjsj=$(json).attr("BJSJ");
				var ywbid=$(json).attr("YWBID");
				$(window).manhuaDialog({"title":"施工许可手续>信息反馈","type":"text","content":"${pageContext.request.contextPath}/jsp/business/qqsx/sgxk/xxfk.jsp","modal":"3"});
			}); 
		});
	});
	//添加或删除异步操作
    ...
    //清空表单 主要是ywid的删除 附件的删除 以及字典选择导致fjlb的变换 字典可用的判断
     ...
     function set_Value(){
 		var json=do_value();
 		setValueByArr(json,["JHSJID","XMMC","BDMC","XMID","BDID","SJWYBH"]);
 		...
 		$("#Q_FJLX").val(sx);
 		$("#FJLX").attr("src","T#fs_dic_tree:dic_code:dic_value:parent_id='1000000000112' and dic_code in("+sx+")");
 		$("#FJLX").html('');
     ...
     <form method="post" id="queryForm"  >
      ...
      <!--可以再此处加入hidden域作为过滤条件 -->
      	...
      	<!-- <TR> -->
			...
			<TD><INPUT type="text" class="span12" kind="text" name="SJWYBH" fieldname="SJWYBH" id="Q_SJWYBH" operation="=" keep="true"></TD>
     ...
     <form method="post" id="SxfjForm"  >
      <table class="B-table" width="100%">
         ...
         <!-- <TR> -->
			...
			<TD><input type="text" class="span12" kind="text" id="BDID" keep="true" ></TD>
			<TD><input type="text" class="span12" kind="text" id="BDMC" fieldname="" keep="true" ></TD>
			<TD><input class="span12" id="BBLSX" type="checkbox" kind="dic" src="SGXKSX" name ="BBLSX" ></TD>
        ...
        <tr>
          ...
          <select class="span12" id="FJLX" name="FJLX" fieldname ="FJLX" kind="dic" placeholder="必填" check-type="required" src="SGXKSX" onchange="change()">
            </select>
          </td>
