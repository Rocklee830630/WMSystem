	...
	$(function() {
		...
		btn.click(function(){
	 	 if($("#demoForm").validationButton()){
				...
				//调用ajax插入
		 		defaultJson.doInsertJson(controllername + "?saveInfo", data1,null,'editHuiDiao');
				}
	 	 });
		//添加关联按钮事件
		...
		$("#xmBtn").click(function(){
			$(window).manhuaDialog({"title":"父节点","type":"text","content":"${pageContext.request.contextPath}/jsp/business/cjjcssxmjsjh/cjjhjdTree.jsp","modal":"2"});
		});
...
function editHuiDiao()
{
	...
	fuyemian.jiedianhuiadd(data2,2);
...
<div class="container-fluid">
 ...
 <form method="post" action="${pageContext.request.contextPath }/insertdemo.xhtml" id="demoForm"  >
      		<table style="width: 100%" class="B-table">
				...
				<tr>
					<th  width="8%" class="right-border bottom-border">父节点</th>
          			<td   width="42%" class="right-border bottom-border " colspan="2" >
          			<input class="span12" style="width:80%"  disabled id="PARENTNAME" type="text" placeholder="必填" fieldname="PARENTNAME" check-type="required" name = "PARENTNAME">
          			  <button class="btn btn-link"  title="请选择父节点" type="button" id="xmBtn" ><i class="icon-edit"></i></button>
       			    </td>
          			<td colspan="3"></td>
				</tr>
				<tr>
				...
				<th width="5%" class="right-border bottom-border">项目性质</th>
					...
					<td  width="10%" class="right-border bottom-border">
						<select kind="dic" src="XMSX" class="span12 4characters"  check-type="required" id="SSXZ" name="SSXZ" fieldname="SSXZ"></select>
					</td>
				</tr>
				<tr>
					<th>年度</th>
