...
<%
	...
	String htid = request.getParameter("htid")==null?"":request.getParameter("htid");
...
$(function() {
	...
	//-根据ID查询合同信息
	//-------------------------------------------
	function getHtInfo(m,n){
		var info = "";
		$.ajax({
			url:controllername2 + "?query&id="+m+"&cxlx="+n,
			data:"",
			dataType:"json",
			async:false,
...
function init(){
	...
	%>
		...
		$.ajax({
			url : controllername+"?query"+idCondition,
			data : data1,
			cache : false,
			async :	false,
			dataType : "json",  
     ...
     <form method="post" id="gcHtglHtForm"  >
      <table class="B-table" width="100%" >
      	<tr style="display:none;">
	      	<td><input type="hidden" id="ID" fieldname="ID" name = "ID"/></TD>
	      	<td><input type="hidden" id="AlerdyHTSJFlag" value = "false"/></TD>
      	</tr>
...
<%--				需计算得出  : 总合同签订价+合同变更--%>
		...
		<tr>
			...
			<td width="17%" class="right-border bottom-border">
				<input id="ZHTJS" value=0 class="span9" style="width:70%;text-align:right;"   name="ZHTJS" fieldname="ZHTJS" type="number" min="0"/><b>(元)</b>
		...
		<h4 id="bzjTitle" class="title" style="display:none;" >履约保证金情况</h4>
			<table id="bzjBody" class="B-table" width="100%" style="display:none;">
				<tr>
        ...
        <tr>
        	<th width="8%" class="right-border bottom-border text-right disabledTh">正式合同</th>
        	<td colspan="3" class="bottom-border right-border">
				<div>
					<table role="presentation" class="table table-striped">
						<tbody fjlb="0702" class="files showFileTab" data-toggle="modal-gallery" data-target="#modal-gallery" onlyView="true"></tbody>
					</table>
				</div>
			</td>
        </tr>
