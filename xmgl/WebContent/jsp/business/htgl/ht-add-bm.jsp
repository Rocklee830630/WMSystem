...
<%
	...
	String dkfs = request.getParameter("dkfs");
	dkfs = (dkfs==null &&"".equals(dkfs)?"":dkfs);
	//获取当前用户信息
		...
		if($("#gcHtglHtForm").validationButton())
		{
		  ...
		  //组成保存json串格式
		    ...
		    if($("#ID").val() == "" || $("#ID").val() == null){
    			...
    			p_oldHtlx = $("#HTLX").val();
    			p_oldBjxs = $("#BJXS").val();
    			var flag = defaultJson.doInsertJson(controllername + "?insert&ywid="+valywid+"&xmxxfrom="+xmxxfrom+"&ids="+$("#jhsjids").val()+"&xmids="+$("#xmids").val()+"&bdids="+$("#bdids").val()+"&pqzxmid="+$("#pqzxmid").val(), data1 , null, 'insertHt');
    			if(flag){
...
//合同修改时， 获取项目信息
		...
		if(flagZTBID){
			...
			$("#sel_xmxx").hide();
			//$("#ZHTQDJ").attr('disabled','disabled');//delete by zhangbr@ccthanking.com 按用户要求：即时关联了招投标，也允许修改合同签订价（下同）
...
//招投标
function xzZtb() {
	$(window).manhuaDialog({"title" : "选择招投标信息","type" : "text","content" : "${pageContext.request.contextPath}/jsp/business/ztb/ztbQuery.jsp?bmht=true","modal":"2"});
}
...
<%--			<th width="8%" class="right-border bottom-border text-right">招标类型</th>--%>
		...
		<tr id="ISSJZB" style="display: none;">
			<th width="8%" class="right-border bottom-border text-right disabledTh">折扣系数</th>
			<td width="17%" class="right-border bottom-border" colspan="3">
				<input  id="ZKXS"   class="span3" type="text"  name="ZKXS" fieldname="ZKXS"    disabled></select>
			</td>
		</tr>
		<tr>
			...
			<td width="20%" class="right-border bottom-border">
	            <select class="span12 person" id="QQDNF" name="QDNF" check-type="required" fieldname="QDNF" operation="="  kind="dic" src="XQZBNF"></select>
	        </td>
		</tr>
		<tr>
...
<%--		</tr>--%>
        ...
        <tr id="zshtTR" style="display:none;">
        	<th width="8%" class="right-border bottom-border text-right">正式合同</th>
        	<td colspan="3" class="bottom-border right-border">
				<div>
					<span class="btn btn-fileUpload addFileSpan" onclick="doSelectFile(this);" fjlb="0702">
						<i class="icon-plus"></i>
						<span>添加文件...</span>
					</span>
					<table role="presentation" class="table table-striped">
