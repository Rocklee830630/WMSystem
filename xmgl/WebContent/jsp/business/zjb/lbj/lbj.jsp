    ...
    function ready() {
		...
		dayi.click(function() {
			...
			if(index1<0){
				...
				$("#resultXML").val($("#DT1").getSelectedRow());
				$(window).manhuaDialog({"title":"拦标价>答疑日期","type":"text","content":"${pageContext.request.contextPath}/jsp/business/zjb/lbj/dyqk.jsp?bdmc="+bdmc,"modal":"2"});
				} 
			}
		});
		  //反馈
	    $("#btnFeedback").click(function(){
	    	...
	    	openJhfkPage(tempJson.JHSJID,"2001","queryForm","DT1",encodeURI(controllername+"?queryLbj"),"4");
	    	}
	    	});
   };
   //点击行获取数据
	...
	//答疑弹出页回显修改
	function dayixiugaihang(sjwybh,nd)
	{
		$.ajax(
		{
			   url : controllername+"?queryLbj",//此处定义后台controller类和方法
		         data : {sjwybh:sjwybh,nd:nd},    //此处为传入后台的数据，可以为json，可以为string，如果为json，那起结构必须和后台接收的bean一致或和bean的get方法名一致，例如｛id：1，name：2｝后台接收的bean方法至少包含String id,String name方法  如果为string，那么可以写为{portal: JSON.stringify(data)}, 后台接收的时候参数可以为String，名字必须和前台保持一致及定义为String portal
...
<app:dialogs/>
    ...
    <div class="B-small-from-table-autoConcise">
      <h4 class="title">拦标价管理<span class="pull-right">
                <app:oPerm url="/jsp/business/zjb/lbj/lbjadd.jsp">
				<button id="addlbj" class="btn" type="button">新增</button>
				</app:oPerm>
				 <app:oPerm url="/jsp/business/zjb/lbj/dyqk.jsp">
				<button id="dayi" class="btn" type="button">答疑</button>
				</app:oPerm>
				 <app:oPerm url="/jsp/business/zjb/lbj/lbjwh.jsp">
          		 <button id="wh" class="btn"  type="button">维护</button>
          		 </app:oPerm>
          		  <app:oPerm url="/jsp/business/jhfk/lbj/jhfkMain.jsp">
          		 <button id="btnFeedback" class="btn" type="button">统筹计划反馈</button>
          		  </app:oPerm>
     ...
     <form method="post" id="queryForm"  >
	  ...
	  <div style="height:5px;"> </div>
        <div class= "overFlowX">
            <table width="100%" class="table-hover table-activeTd B-table" id="DT1" type="single" editable="0" printFileName="拦标价管理">
                <thead>
                    <tr>	
