...
<script type="text/javascript" charset="utf-8">
  ...
  var qujian='<%=qujian%>';
  var iszxgs=<%=iszxgs%>;
  var controllername= "${pageContext.request.contextPath }/zhaoJiaBuZhangController.do";
	...
	$(function() {
		...
		btn.click(function() {
			...
			//调用ajax插入
			var qujian1=qujian.replace(/%/g,"%25");
			var condition = "nd="+nd+"&qujian="+qujian1+"&iszxgs=1";
			defaultJson.doQueryJsonList(controllername+"?queryLbj&"+condition,data,DT1,null,false);
		});
		//清空查询条件
		var clear=$("#clear");
    ...
    function ready() {
		...
		//调用ajax插入
		var qujian1=qujian.replace(/%/g,"%25");
   		var condition = "nd="+nd+"&qujian="+qujian1+"&iszxgs="+iszxgs+"";
		defaultJson.doQueryJsonList(controllername+"?queryLbj&"+condition,data,DT1,null,false);
   };
   //点击行获取数据
    ...
    <div class="B-small-from-table-autoConcise">
     ...
     <form method="post" id="queryForm"  >
      ...
      <TR  style="display:none;">
			...
			<%if(num!=null)
								{%>
									<INPUT id="num" type="text" class="span12" kind="text" fieldname="rownum" value=<%=num%> operation="<=" keep="true"/>
							<%
								}else
								{ %>	
