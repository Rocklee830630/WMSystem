...
<app:base/>
<title>项目管理公司-招标条件管理</title>
<script type="text/javascript" charset="utf-8">
//请求路径，对应后台RequestMapping
var controllername= "${pageContext.request.contextPath }/common_yw/ztbtjgl/ZtbtjglController.do";
var p_selectIndex = '0';
//页面初始化
$(function() {	
	init();
	...
	$("#btnExpExcel").click(function() {
		 if(exportRequireQuery($("#ztbtjgl"))){//该方法需传入表格的jquery对象
		      printTabList("ztbtjgl","gcjlgl.xls","XMBH,XMID,BDID,HTID,JLYF,DYJLSDZ","2,1");
		  }
	});
	//按钮绑定事件（清空）
    ...
    /* $('input[name="subBox').bind("click",function(event){
    	//"tr_click",
    	//alert(22)
...
function insert_update(flag){
	...
	$('#ConfirmYesButton').one("click",function(){ 
		...
		$("[name=subBox]:checkbox:checked").each(function(){ 
			...
			$("#ztbtjgl").setSelect(hang);
			//生成json串
			var data=$("#ztbtjgl").getSelectedRow();
			var obj=convertJson.string2json1(data);
			data=JSON.stringify(obj);
			//组成保存json串格式
			var data1 = defaultJson.packSaveJson(data);
			//调用ajax插入
			actionFlag = defaultJson.doUpdateJson(controllername + "?insert&flag="+flag,data1, ztbtjgl);
			if(actionFlag==false){
				return false;
			}
		});
...
function tr_click(obj,tabListid){
	var index= $("#ztbtjgl").getSelectedRowIndex();
	var num=0;
	$('input[name="subBox"]').each(function(){
	//alert($(this).prop("checked")+"  "+num+"  "+index);
		if(num==index)
		{
...
function queryList(){
	//生成json串
	var data = combineQuery.getQueryCombineData(queryForm,frmPost,ztbtjgl);
	//调用ajax插入
	defaultJson.doQueryJsonList(controllername+"?query",data,ztbtjgl);
}


//详细信息
function rowView(index){
	var obj = $("#ztbtjgl").getSelectedRowJsonByIndex(index);
...
<p></p>
  <div class="row-fluid">
     <div class="B-small-from-table-autoConcise">
      <h4 class="title">招标条件管理
      	<span class="pull-right">
   			<button class="btn" onclick="insert_update(1)" type="button">监理具备</button>
   			<button class="btn" onclick="insert_update(2)" type="button">监理暂缓</button>
   			<button class="btn" onclick="insert_update(3)" type="button">施工具备</button>		
   			<button class="btn" onclick="insert_update(4)" type="button">施工暂缓</button>
...
<div style="height:5px;"> </div>
	<div class="overFlowX">
	<table class="table-hover table-activeTd B-table" id="ztbtjgl" width="100%" withoutAlertFlag="true" type="single" pageNum="10" printFileName="工程计量">
		<thead>
			<tr>
				...
				<th fieldname="JLZT" tdalign="center">&nbsp;&nbsp;监理条件</th>
				<th fieldname="SGZT" tdalign="center">&nbsp;施工条件&nbsp;</th>
			</tr>
		</thead>
		<tbody>
           </tbody>
