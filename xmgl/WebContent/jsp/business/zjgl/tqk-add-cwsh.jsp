...
<%
	...
	String qlx = request.getParameter("qlx");
%>
...
$(function() {
	...
	$("#btnSave").click(function() {
		...
		{
			if($("#gcZjglTqkmxList tbody tr").size()>0){
	...
	$("#btnUpdateMxSave").click(function() {
			...
			if(tharrays != null&&indexArryChange.length>0){
...
function init(){
	...
	if(type == "insert"){
		...
		//调用ajax插入
		defaultJson.doQueryJsonList(controllernameTqkMx+"?query&opttype=tocwedit",dataMx,gcZjglTqkmxList);
      ...
      <table class="B-table" width="100%" >
		...
		<tr>
			...
			<td width="17%" class="right-border bottom-border">
				<select type="text" class="span12"  fieldname="SQDW" name="SQDW" id="SQDW" kind="dic" src="T#VIEW_YW_ORG_DEPT:ROW_ID:DEPT_NAME" disabled="disabled"></select></td>
			</td>
		...
		<thead>
			<tr>
				...
				<th fieldname="ZXHTJ" colindex=5 tdalign="right" >&nbsp;最新合同价(元)&nbsp;</th>
				 <%
			    if("14".equals(qlx)||"16".equals(qlx)){
			    %>
				<th fieldname="CSZ" colindex=6 tdalign="right" >&nbsp;财审值(元)&nbsp;</th>
				<%
					}
				%>
				<%if("16".equals(qlx)){//工程类显示审计值
					%>
						<th fieldname="SJZ" colindex=7 tdalign="right" >&nbsp;审计值(元)&nbsp;</th>
						<th fieldname="JZQR" colindex=8tdalign="right" >&nbsp;监理确认计量值&nbsp;</th>	
					<%
				} %>
				<%if("15".equals(qlx)){//工程类显示结算值
					%>
						<th fieldname="JSZ" colindex=9 tdalign="right" >&nbsp;结算值(元)&nbsp;</th>
					<%
				} %>
				
				<th fieldname="YBF" colindex=10 tdalign="right" >&nbsp;已拔付(元)&nbsp;</th>
				<th fieldname="DBF" colindex=11 tdalign="right" CustomFunction="doDBF">&nbsp;待拔付(元)&nbsp;</th>
				<th fieldname="BCSQ" colindex=12 tdalign="right">&nbsp;部门申请值(元)&nbsp;</th>
				<th fieldname="CWSHZ" colindex=13 tdalign="right" type="text">&nbsp;财务确认申请值(元)&nbsp;</th>
				<th fieldname="LJBF" colindex=14 tdalign="right" >&nbsp;累计拔付(元)&nbsp;</th>
				<th fieldname="AHTBFB" colindex=15 tdalign="right" >&nbsp;按合同付款比例(%)&nbsp;</th>
				 <%
			    if(type.equals("update")){
				%>
				<th fieldname="BZ" colindex=16 tdalign="center" maxlength="500" type="text">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&emsp;&emsp;备注&emsp;&emsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
				<%
					}else{
				%>
				<th fieldname="BZ" colindex=16 tdalign="center" maxlength="300" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&emsp;&emsp;备注&emsp;&emsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
				<%
					}
				%>
				<%if("16".equals(qlx)){//只有工程类提请款才显示甲供材
					%>
						
						<th fieldname="AJLFKB" colindex=17 tdalign="right" >&nbsp;按计量付款比例(%)&nbsp;</th>	
						<th fieldname="JGC" colindex=18 tdalign="right" >&nbsp;甲供材(元)&nbsp;</th>	
					<%
				} %>
