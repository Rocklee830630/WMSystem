...
$(function() {
	...
	$("#btnInsert").click(function() {
		$(window).manhuaDialog({"title":"工程甩项管理>新增","type":"text","content":"${pageContext.request.contextPath}/jsp/business/gcb/gcsx/gcsx_xmwh.jsp?type=insert&fqlx=1","modal":"1"});
	});
	//按钮绑定事件(修改)
	$("#btnUpdate").click(function() {
		...
		$(window).manhuaDialog({"title":"工程甩项管理>修改","type":"text","content":"${pageContext.request.contextPath}/jsp/business/gcb/gcsx/gcsx_xmwh.jsp?type=update&fqlx=1","modal":"1"});
	});
	//按钮绑定事件（清空）
    ...
    $("#sp_btn").click(function() {
    	...
    	if(obj.EVENTSJBH ==0) {//流程发起时，用发起的页面
    		...
    		// templateid 一个为27，一个为26；operationoid一个为43480，一个为43481    add  by  xhb
 	    	if(sfpq == 1){
 	    		var wsActionURL = "/xmgl/PubWS.do?getXMLPrintAction|templateid=26|isrefresh=1|isEdit=0|ywlx="+ywlx+"|sjbh="+sjbh+"|rowid="+Math.random()+".mht";
 	            wsActionURL =  "/xmgl/jsp/framework/print/pubPrint.jsp?param="+wsActionURL+
 	            		"&sjbh="+sjbh+"&ywlx="+ywlx+"&temlateid=26&isEdit=0&operationoid=43483&processtype=1";
 	            $(window).manhuaDialog({"title":"项目管理公司甩项申请会签单（有拆迁）","type":"text","content":wsActionURL,"modal":"1"});

 	    	}else{
 	    		var wsActionURL = "/xmgl/PubWS.do?getXMLPrintAction|templateid=27|isrefresh=1|isEdit=0|ywlx="+ywlx+"|sjbh="+sjbh+"|rowid="+Math.random()+".mht";
 	            wsActionURL =  "/xmgl/jsp/framework/print/pubPrint.jsp?param="+wsActionURL+
 	            		"&sjbh="+sjbh+"&ywlx="+ywlx+"&temlateid=27&isEdit=0&operationoid=43484&processtype=1";
 	            $(window).manhuaDialog({"title":"项目管理公司甩项申请会签单（无拆迁）","type":"text","content":wsActionURL,"modal":"1"});
     ...
     <div class="B-small-from-table-autoConcise">
      <h4 class="title">工程甩项
      	<span class="pull-right">
      		<app:oPerm url="jsp/business/gcb/gcsx/gcsx_xmwh.jsp?type=insert&fqlx=1">
      			<button id="btnInsert" class="btn" type="button">新增</button>
      		</app:oPerm>
      		<app:oPerm url="jsp/business/gcb/gcsx/gcqs_xmwh.jsp?type=update&fqlx=1">
      			<button id="btnUpdate" class="btn" type="button">修改</button>
      		</app:oPerm>
      		<app:oPerm url="jsp/framework/common/aplink/defaultArchivePage.jsp?type=gcsx">
      			<button id="sp_btn" class="btn"  type="button">呈请审批</button>
      		</app:oPerm>
      		<app:oPerm url="jsp/business/gcb/gcsx/gcsx_xmwh.jsp?type=delete&fqlx=1">
