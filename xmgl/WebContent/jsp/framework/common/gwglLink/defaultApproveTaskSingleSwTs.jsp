...
var xuanzhong = '';
	<%String taskid = Pub.val(request, "taskid");
			...
			String isRead = Pub.val(request, "isRead");
			String ywid = "";
			...
			try {
				...
				String cjrsql = "select cjrid,cjdwdm from AP_PROCESSINFO t where t.eventid='" + sjbh + "'";
...
function doSubmit(jsrStr,jsdwStr)
{
	...
	try{
		...
		//data1+= '&result=1&resultDscr='+encodeURI(document.getElementById('mind').value)+'&fjbh='+vfjbh+'&spFieldname='+spFieldname;
		jsonObj["result"] = 1;
...
function customOperation()
{
	...
	var jsrYj = document.getElementById("mind").value;
    var strActionUrl = "<%=request.getContextPath()%>/TaskBackAction.do?customOperationAction";
    
	var data1 = '&eventid=<%=sjbh%>&ywlx=<%=ywlx%>&spbh=<%=spbh%>&taskId=<%=taskid %>&jsrYj='+jsrYj;
		$.ajax({
			type : 'post',
			url : strActionUrl,
			data : data1,
			...
			success : function(result) {
				...
				var data = Form2Json.formToJSON(mindForm);
				//组成保存json串格式
				var data1 = defaultJson.packSaveJson(data);
				defaultJson.doInsertJson("<%=request.getContextPath()%>/TaskBackAction.do?insertApProcessWsWhenEnd&&sjbh=<%=sjbh%>&ywlx=<%=ywlx%>&jsrYj="+jsrYj, data1, null);
 ...
 <div class="row-fluid">
        <div class="B-small-from-table-autoConcise">
            ...
            <div class="span7" style="height:100%;overflow:hidden;"  id="wsTemplateDiv">
             	<iframe name="iFrame" width="100%" height="90%" src="/xmgl/PubWS.do?getXMLPrintAction&templateid=<%=ws_template_id%>&sjbh=<%=sjbh%>&ywlx=<%=ywlx%>&isSp=1" FRAMEBORDER=0 SCROLLING= ></iframe>
			</div>
    		...
    		<div class="span5" >
		    		<h4 class="title">
						...
						<%
 						...
 						%>
						<!-- <button type="button" class="btn" name="chakan" onClick="chakan()">查看办理意见</button> -->
						<%
							// 当选择【办】的情况是，会返回多条待办信息。这里是判断待办信息是否为最后一条，当时最后一条待办信息时，则隐藏【结束此待办】按钮
							String dbsSql = "select count(*) cnt from AP_TASK_SCHEDULE t "
									+ "where t.ywlx='200502' and spbh = '"+ spbh +"'  "
									+ "and wcsj is null ORDER BY CJSJ DESC";
							String[][] dbs = DBUtil.query(dbsSql);
							if(!"1".equals(dbs[0][0])) {
						%>
						<button type="button" class="btn" name="btnJssp" onClick="comfirmFinishThis();">结束此待办</button> 
						<%
							}
						%>
						...
						<%
 						...
 						%>
 						<button type="button" class="btn" id="btnfqr" name="btnFhfqr">提交</button> 
 						<%
 							}
 						%>
						...
						<table class="B-table" width="100%">
							...
							<%
					//		System.out.println(fqrid+"-------------------------------------------"+useraccount);
								if(useraccount.equals(fqrid)) {
							%>
						   <tr>
						   ...
						   <tr>
								<th class="right-border bottom-border" style="font-size:14px;">选择办阅</th>
								<td class="bottom-border right-border">
			         				办 <input type="radio" id="isRead" name ="isRead" value="1" checked="checked"/>&nbsp;&nbsp;&nbsp;
			         				阅 <input type="radio" id="isRead" name ="isRead" value="2"/>
			       	 			</td>
						   </tr>
						   <%
								}
