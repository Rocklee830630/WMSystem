  ...
  function setPageHeight(){
	...
	$(function() {
		...
		btn.click(function() {
			...
			//调用ajax插入
			defaultJson.doQueryJsonList(controllername+"?bm_monitor&start="+start+"&end="+end+"&dic="+dic,data,DT1,null,false);
			doDic();
				});
		//清空
		var clean=$("#clean");
		...
		$("#excel").click(function(){
			if(exportRequireQuery($("#DT1"))){//该方法需传入表格的jquery对象
				printTabList("DT1","bmbbxx.xls","BMMC,JRZL,JRMX,JRRYMX,LJZL,LJMX,LJRYMX","3,1");
			  }
		});
    ...
    function ready(ready) {
  		...
  		var end=$("#END").val();
   		var data = combineQuery.getQueryCombineData(queryForm,frmPost,DT1);
		//调用ajax插入
		defaultJson.doQueryJsonList(controllername+"?bm_monitor&start="+start+"&end="+end,data,DT1,null,false);
		doDic();
   };
   //列名显示
   function showTr(){
		...
		<form method="post" id="bmForm" style="width:100%; height:100px;display:none">
				...
				<tr>
					<td class="bottom-border">
						<input class="span12" type="checkbox" id="BM" name ="BM" kind="dic" src="T#VIEW_YW_ORG_DEPT:ROW_ID:decode(BMJC,null,DEPT_NAME,BMJC):DEPT_PARANT_ROWID !='0' and ROW_ID not in ('100000000008','100000000000')" >
				</tr>
			</table>
		</form>
				...
				<table width="100%" class="table-hover table-activeTd B-table" id="DT1" type="single" noPage="true" pageNum="2000">
					<thead>
						<tr>
							...
							<th colspan="3" id="jr">今日录入</th>
							<th colspan="3" id="lj">累计录入</th>
						</tr>
						<tr>
							<th fieldname="JRZL" tdalign="right" colindex=3>&nbsp;录入总量&nbsp;</th>
							<th fieldname="JRMX" tdalign="right" colindex=4 CustomFunction="showJrmx">&nbsp;业务模块明细&nbsp;</th>
							<th fieldname="JRRYMX" tdalign="right" colindex=5 CustomFunction="showJrrymx">&nbsp;个人明细&nbsp;</th>
							<th fieldname="LJZL" tdalign="right" colindex=6>&nbsp;录入总量&nbsp;</th>
							<th fieldname="LJMX" tdalign="right" colindex=7 CustomFunction="showLjmx">&nbsp;业务模块明细&nbsp;</th>
							<th fieldname="LJRYMX" tdalign="right" colindex=8 CustomFunction="showLjrymx">&nbsp;个人明细&nbsp;</th>
						</tr>
					</thead>
					<tbody></tbody>
				</table>
